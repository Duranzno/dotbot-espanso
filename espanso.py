...
class Espanso(dotbot.Plugin):
    _espansoDirective = "espanso"

    def can_handle(self, directive):
        return directive in (self._espansoDirective)

    def handle(self, directive, data):
        if directive == self._espansoDirective:
            return self._process_data("espanso install ", data)
        raise ValueError('espanso cannot handle directive %s' % directive)
    ...
    def _install(self, install_cmd, packages_list):
        ...
        with open(os.devnull, 'w') as devnull:
            ...
            for package in packages_list:
                ...
                # if isInstalled != 0:
                log.info("Installing espanso package: %s" % package)
